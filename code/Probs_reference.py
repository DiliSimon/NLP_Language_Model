...
class LanguageModel:
    ...
    def make(cls, smoother: str, lexicon: Path) -> "LanguageModel":
        ...
        if smoother_name == "uniform":
            ...
            assert lambda_ is not None
            return ImprovedLanguageModel(lambda_, lexicon)
        else:
...
class LogLinearLanguageModel(LanguageModel):
    def __init__(self, c: float, lexicon: Path) -> None:
        ...
        #self.X = np.random.rand(self.dim, self.dim)
        #self.Y = np.random.rand(self.dim, self.dim)
    ...
    def prob(self, x: str, y: str, z: str) -> float:
        ...
        uxyz = math.exp(np.dot(np.dot(xv, self.X),zv)+np.dot(np.dot(yv,self.Y),zv))
        zxy = 0
        for name in self.vocab:
            name = self.replace_missing(name)
            item = np.array(self.vectors[name])
            zxy += math.exp(np.dot(np.dot(xv, self.X),item)+np.dot(np.dot(yv,self.Y),item))
        return uxyz/zxy

    def ZXY(self, x: str, y: str) -> float:
        assert self.vocab is not None
        x = self.replace_missing(x)
        y = self.replace_missing(y)
    ...
    def UXYZ(self, x: str, y: str, z: str):
        x = self.replace_missing(x)
        y = self.replace_missing(y)
        z = self.replace_missing(z)
    ...
    def prob_fast(self, x: str, y: str, z: str):
        uxyz = self.UXYZ(x,y,z)
        zxy = self.ZXY(x,y)
        return uxyz/zxy
    ...
    def train(self, corpus: Path) -> List[str]:
        ...
        print(f"Training from corpus {corpus.name}")
        print(f"Vocabulary size is {self.vocab_size} types indluding OOV and EOS")
        ...
        for e in range(epochs):
            ...
            for i in range(2, len(tokens_list)):
                ...
                ## Get gradient
                '''
                gradient_X = [[0.0 for _ in range(self.dim)] for _ in range(self.dim)]
                gradient_Y = [[0.0 for _ in range(self.dim)] for _ in range(self.dim)]

                for row in range(self.dim):
                    for col in range(self.dim):
                        gradient_X[row][col] = xv[row]*zv[col]-2*self.c*self.X[row][col]/self.N
                        gradient_Y[row][col] = yv[row]*zv[col]-2*self.c*self.Y[row][col]/self.N
                        for name in self.vocab:
                            name = self.replace_missing(name)
                            namev = np.array(self.vectors[name])
                            gradient_X[row][col] -= (self.UXYZ(x,y,name)/zxy)*(xv[row]*namev[col])
                            gradient_Y[row][col] -= (self.UXYZ(x,y,name)/zxy)*(xv[row]*namev[col])

                '''

                gradient_X = np.zeros([self.dim,self.dim])
                gradient_Y = np.zeros([self.dim,self.dim])
                gradient_X += np.dot(xv.reshape(xv.shape[0],1), zv.reshape(1,zv.shape[0]))
                gradient_Y += np.dot(yv.reshape(yv.shape[0],1), zv.reshape(1,zv.shape[0]))
                for name in self.vocab:
                    name = self.replace_missing(name)
                    namev = np.array(self.vectors[name])
                    gradient_X -= (self.UXYZ(x,y,name)/zxy)*np.dot(xv.reshape(xv.shape[0],1), namev.reshape(1,namev.shape[0]))
                    gradient_Y -= (self.UXYZ(x,y,name)/zxy)*np.dot(yv.reshape(yv.shape[0],1), namev.reshape(1,namev.shape[0]))
                ...
                check = 0
                if check == 1:
                    realX = np.array(self.X)
                    realY = np.array(self.Y)
                    #delta = np.random.random((self.dim,self.dim))*(10**(-6))
                    aaa = np.random.randint(low = 0, high = 9, size = 1)
                    bbb = np.random.randint(low = 0, high = 9, size = 1)
                    delta = np.zeros([10,10])
                    aaa = [1]
                    bbb = [1]
                    delta[aaa[0]][bbb[0]] = 10**(-6)
                    tX = realX + delta
                    self.X = tX.tolist()
                    Fi_delta = math.log(self.prob(x,y,z))
                    self.X = realX.tolist()
                    error = sum(sum(delta*gradient_X))
                    print((Fi+error-Fi_delta))
                F += Fi
                t += 1
            F -= self.c * (np.linalg.norm(temp_X)**2 + np.linalg.norm(temp_Y)**2)
            F = F/self.N
            print(f"epoch {e+1}: F={F}")
        print(f"Finish training on {self.N} tokens")
...
class ImprovedLanguageModel(LanguageModel):
    def __init__(self, c: float, lexicon: Path) -> None:
        super().__init__()
        ...
        self.X: Any = None
        self.Y: Any = None
        self.angle_xz_coef: Any = None
        self.angle_yz_coef: Any = None
    ...
    def UXYZ(self, x: str, y: str, z: str):
        ...
        uxyz = math.exp(np.dot(np.dot(xv, self.X),zv)+np.dot(np.dot(yv,self.Y),zv)+ self.angle_xz_coef*np.dot(xv,zv)/(np.linalg.norm(xv)*np.linalg.norm(zv)) + self.angle_yz_coef*np.dot(yv,zv)/(np.linalg.norm(yv)*np.linalg.norm(zv)))
        return uxyz

    def train(self, corpus: Path) -> List[str]:
        print('Using probs_reference')
        """Read the training corpus and collect any information that will be needed
        by the prob function later on.  Tokens are whitespace-delimited.
        ...
        self.X = [[0.0 for _ in range(self.dim)] for _ in range(self.dim)]
        self.Y = [[0.0 for _ in range(self.dim)] for _ in range(self.dim)]
        self.angle_xz_coef = 0
        self.angle_yz_coef = 0
        ...
        for e in range(epochs):
            ...
            for i in range(2, len(tokens_list)):
                ...
                gradient_X += np.dot(xv.reshape(xv.shape[0],1), zv.reshape(1,zv.shape[0]))
                gradient_Y += np.dot(yv.reshape(yv.shape[0],1), zv.reshape(1,zv.shape[0]))
                gradient_angle_xz += np.dot(xv,zv)/(np.linalg.norm(xv)*np.linalg.norm(zv))
                gradient_angle_yz += np.dot(yv,zv)/(np.linalg.norm(yv)*np.linalg.norm(zv))

                for name in self.vocab:
                    name = self.replace_missing(name)
                    namev = np.array(self.vectors[name])
                    coef = self.UXYZ(x,y,name)/zxy
                    gradient_X -= coef * np.dot(xv.reshape(xv.shape[0],1), namev.reshape(1,namev.shape[0]))
                    gradient_Y -= coef * np.dot(yv.reshape(yv.shape[0],1), namev.reshape(1,namev.shape[0]))
                    gradient_angle_xz -= coef * np.dot(xv,namev)/(np.linalg.norm(xv)*np.linalg.norm(namev))
                    gradient_angle_yz -= coef * np.dot(yv,namev)/(np.linalg.norm(yv)*np.linalg.norm(namev))

                gradient_X -= 2*self.c*np.array(self.X)/self.N
                gradient_Y -= 2*self.c*np.array(self.Y)/self.N
                gradient_angle_xz -= 2*self.c*np.array(self.angle_xz_coef)/self.N
                gradient_angle_yz -= 2*self.c*np.array(self.angle_yz_coef)/self.N
                ...
                self.X = temp_X.tolist()
                self.Y = temp_Y.tolist()
                self.angle_xz_coef += gamma*gradient_angle_xz
                self.angle_yz_coef += gamma*gradient_angle_yz
